...
try:
    ...
    from quote_builder import quote_builder
...
def main():
    ...
    # Tab 4: Quote Builder & Simulation
    with tab4:
        if not setup_data:
            st.warning("⚠️ Please complete Job Setup first")
        else:
            # Create sub-tabs for Quote Builder and Simulation
            quote_tab, sim_tab = st.tabs(["💰 Quote Builder", "🎯 Simulation"])
            
            with quote_tab:
                try:
                    # Get current data for quote building
                    parts_per_bar = setup_data.get('parts_per_bar', 1)
                    bar_weight = setup_data.get('bar_weight', 1.0)
                    cycle_time = st.session_state.get('cycle_time_from_tab2', setup_data.get('cycle_time', 1.6))
                    
                    # Run quote builder
                    quote_data = quote_builder.quote_breakdown_section(parts_per_bar, bar_weight, cycle_time)
            ...
            with sim_tab:
                try:
                    ...
                    # Run simulation
                    sim_results = simulation_system.simulation_section(setup_data, spindle_ops, threading_data, quote_data)
    ...
    # Tab 5: Reference Charts
    with tab5:
        try:
            reference_charts.reference_charts_section()
        except Exception as e:
